<div class="space-y-6">
    <flux:heading size="xl">Kelola Mata Pelajaran</flux:heading>
    ...
    <form wire:submit="save" class="grid gap-4 md:grid-cols-3">
        <flux:field><flux:label>Nama</flux:label><flux:input wire:model="name" /><flux:error name="name" /></flux:field>
    ...
    <div class="grid gap-3 md:grid-cols-2">
        @foreach ($subjects as $subject)
            <flux:card wire:key="subject-{{ $subject->id }}">
                <div class="font-semibold">{{ $subject->name }}</div>
                <flux:text>{{ $subject->code }} · {{ $subject->modules_count }} modul</flux:text>
            </flux:card>
        @endforeach
    </div>
</div>
